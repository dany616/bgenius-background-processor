...
class BackgroundRemover {
    ...
    async removeBackground(imageBuffer, options = {}) {
        ...
        try {
            ...
            if (mergedOptions.model === 'removebg') {
                ...
                processedBuffer = await this.removeBackgroundWithRemoveBg(imageBuffer, mergedOptions.apiKey);
            }
            else {
    ...
    async removeBackgroundWithTensorFlow(imageBuffer) {
        try {
            ...
            // Convert to buffer
            const processedImageData = await tf.browser.toPixels(maskedImage);
            ...
            throw new Error(`TensorFlow processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
...
export { BackgroundGenerator, BackgroundProcessor, BackgroundRemover, DEFAULT_OPTIONS, SUPPORTED_FORMATS, createImageProcessor, validateImage };
