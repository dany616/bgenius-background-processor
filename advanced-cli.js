...
async function simulateBackgroundRemoval(inputPath, outputPath, options = {}) {
  ...
  try {
    ...
    console.log(
      chalk.blue(`📊 원본 파일 크기: ${(inputStats.size / 1024).toFixed(2)} KB`)
    );
...
async function simulateBackgroundGeneration(
  inputPath,
  outputPath,
  prompt,
  options = {}
...
program
  ...
  .action(async (input, options) => {
    ...
    const success = await simulateBackgroundGeneration(
      input,
      options.output,
      prompt,
      {
...
program
  ...
  .action(async () => {
    ...
    if (answers.action === 'remove') {
      ...
      await simulateBackgroundGeneration(
        answers.input,
        answers.output,
        answers.prompt
      );
      ...
      const removeSuccess = await simulateBackgroundRemoval(
        answers.input,
        tempFile
      );
...
program.on('command:*', () => {
  ...
  console.log(
    chalk.yellow('💡 --help를 실행하여 사용 가능한 명령을 확인하세요.')
  );
  process.exit(1);
});
