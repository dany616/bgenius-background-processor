...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <div className={`min-h-screen ${theme === 'dark' ? 'bg-gradient-to-b from-gray-900 to-gray-800 text-white' : 'bg-gradient-to-b from-gray-50 to-white'}`}>
        {/* Hero Section */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
          <div className="text-center">
            <h1 className={`text-4xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} sm:text-5xl md:text-6xl`}>
            Bgenius.AI
            </h1>
        ...
        <div className={`py-16 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {/* Feature 1 */}
              <div className={`relative p-6 rounded-xl shadow-sm border ${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600' : 'bg-white border-gray-100'}`}>
                <div className="w-12 h-12 flex items-center justify-center rounded-lg bg-blue-100 text-blue-600">
                  <FiUpload className="w-6 h-6" />
                </div>
                <h3 className={`mt-4 text-lg font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>간편한 업로드</h3>
                <p className={`mt-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
                  URL 및 드래그 앤 드롭으로 손쉽게 이미지를 업로드하고 사진을 편집하세요
                </p>
              </div>
              ...
              <div className={`relative p-6 rounded-xl shadow-sm border ${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600' : 'bg-white border-gray-100'}`}>
                ...
                <h3 className={`mt-4 text-lg font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>AI 배경 변경</h3>
                <p className={`mt-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
              ...
              <div className={`relative p-6 rounded-xl shadow-sm border ${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600' : 'bg-white border-gray-100'}`}>
                ...
                <h3 className={`mt-4 text-lg font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>빠른 속도</h3>
                <p className={`mt-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
                  간편하게 업로드한 사진을 30초내로 누끼를 따주고 배경을 변경해줍니다
                </p>
              </div>
            </div>
          </div>
