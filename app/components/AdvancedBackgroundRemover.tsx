...
const AdvancedBackgroundRemover: React.FC<AdvancedBackgroundRemoverProps> = ({
  ...
  useEffect(() => {
    ...
    const processImage = async () => {
      try {
        ...
        // 배경 제거를 위한 마스크 생성
        setProgress(40);
        const mask = createBackgroundMask(imageData, canvas.width, canvas.height);
        setProgress(70);
  ...
  // 배경/전경 분리를 위한 함수
  const createBackgroundMask = (imageData: ImageData, width: number, height: number): Uint8ClampedArray => {
    const data = imageData.data;
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        for (const bgColor of backgroundColors) {
          if (colorDistance(pixelColor, bgColor) < 30) { // 색상 거리 임계값
            isBackground = true;
            break;
          }
        }
  ...
  const findDominantColors = (colors: { r: number, g: number, b: number }[], k: number): { r: number, g: number, b: number }[] => {
  ...
  // 마스크 정제 (노이즈 제거 및 강화)
  const refineMask = (mask: Uint8ClampedArray, width: number, height: number): Uint8ClampedArray => {
  ...
  const floodFill = (mask: Uint8ClampedArray, width: number, height: number): void => {
  ...
  const applyMorphologicalOperation = (
    mask: Uint8ClampedArray, width: number, height: number, operation: 'erode' | 'dilate', radius: number
  ): void => {
    ...
    <div className="w-full">
      {isLoading && (
        <div className="mt-4 mb-8 w-full">
          ...
          <p className="text-center mt-2 text-gray-600">고급 배경 제거 처리 중... {progress}%</p>
        </div>
      )}
