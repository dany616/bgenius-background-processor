  ...
  const generateBackground = async () => {
    ...
    try {
      ...
      if (!apiResponse.ok) {
        ...
        throw new Error(`배경 생성에 실패했습니다. 오류 코드: ${apiResponse.status}`);
      }
      ...
      const errorMsg = error instanceof Error ? error.message : '배경 생성 중 오류가 발생했습니다.';
    ...
    <div className="w-full">
      ...
      {isLoading && (
        <div className="mt-4 mb-6">
          ...
          <p className="text-center mt-2 text-gray-600">BRIA API에서 배경 생성 중...</p>
        </div>
      )}
    </div>
  );
