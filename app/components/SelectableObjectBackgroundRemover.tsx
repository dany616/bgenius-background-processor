...
const OBJECT_CLASSES_KO: Record<string, string> = {
  ...
  'zebra': '얼룩말', 'giraffe': '기린', 'backpack': '백팩', 'umbrella': '우산', 
  'handbag': '핸드백', 'tie': '넥타이', 'suitcase': '여행 가방', 'frisbee': '프리스비', 
  'skis': '스키', 'snowboard': '스노보드', 'sports ball': '스포츠 공', 
  'kite': '연', 'baseball bat': '야구 방망이', 'baseball glove': '야구 글러브', 
  'skateboard': '스케이트보드', 'surfboard': '서핑보드', 'tennis racket': '테니스 라켓', 
  'bottle': '병', 'wine glass': '와인 잔', 'cup': '컵', 
  ...
  'potted plant': '화분', 'bed': '침대', 'dining table': '식탁', 'toilet': '변기', 
  'tv': 'TV', 'laptop': '노트북', 'mouse': '마우스', 'remote': '리모컨', 
  'keyboard': '키보드', 'cell phone': '휴대폰', 'microwave': '전자레인지', 'oven': '오븐', 
  'toaster': '토스터', 'sink': '싱크대', 'refrigerator': '냉장고', 
  'book': '책', 'clock': '시계', 'vase': '꽃병', 'scissors': '가위', 
  'teddy bear': '테디 베어', 'hair drier': '헤어 드라이어', 'toothbrush': '칫솔'
};

const SelectableObjectBackgroundRemover: React.FC<SelectableObjectBackgroundRemoverProps> = ({
  imageUrl,
  ...
  const [detectionStage, setDetectionStage] = useState<'init' | 'detecting' | 'detected' | 'processing' | 'complete'>('init');
  ...
  const drawBoundingBoxes = () => {
    if (!previewCanvasRef.current || !imageRef.current || !imageRef.current.complete) return;
    ...
    detectedObjects.forEach((prediction, index) => {
      ...
      // 객체 클래스 라벨 표시
      ctx.fillStyle = isSelected ? 'rgba(0, 255, 0, 0.7)' : 'rgba(255, 0, 0, 0.7)';
  ...
  const processSelectedObjects = async () => {
    ...
    try {
      ...
      const selectedClasses = selectedObjectIds.map(index => ({
        ...
        koName: OBJECT_CLASSES_KO[detectedObjects[index].class] || detectedObjects[index].class
      }));
  ...
  useEffect(() => {
    ...
    const initializeDOM = () => {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        const checkElements = () => {
          ...
          const containerElement = document.getElementById('hidden-elements-container');
          ...
          if (!containerElement || !previewContainer) {
            ...
            if (retryCount < maxRetries) {
              ...
              reject(new Error('DOM 요소 컨테이너를 찾을 수 없습니다. 페이지를 새로고침해 주세요.'));
            }
            return;
          }
          ...
          if (!imageRef.current) {
            ...
            if (existingImg) {
              ...
              img.onerror = () => onError('이미지를 로드하는 중 오류가 발생했습니다.');
          ...
          if (!canvasRef.current) {
            const existingCanvas = containerElement.querySelector('canvas:nth-of-type(1)');
            if (!existingCanvas) {
          ...
          if (!outputCanvasRef.current) {
            const existingOutputCanvas = containerElement.querySelector('canvas:nth-of-type(2)');
          ...
          if (!previewCanvasRef.current) {
            const existingPreviewCanvas = previewContainer.querySelector('canvas');
          ...
          setTimeout(() => {
            ...
            if (!canvasRef.current) {
              const canvas = containerElement.querySelector('canvas:nth-of-type(1)');
              if (canvas) {
                // @ts-ignore
            ...
            if (!outputCanvasRef.current) {
              const outputCanvas = containerElement.querySelector('canvas:nth-of-type(2)');
              if (outputCanvas) {
                // @ts-ignore
            ...
            // 모든 요소가 초기화되었는지 확인
            if (imageRef.current && canvasRef.current && outputCanvasRef.current && previewCanvasRef.current) {
              console.log('모든 DOM 요소 초기화 완료');
              resolve();
            } else {
              retryCount++;
              if (retryCount < maxRetries) {
                console.log(`DOM 요소 초기화 재시도 (${retryCount}/${maxRetries})...`);
                setTimeout(checkElements, retryDelay);
              } else {
                reject(new Error('DOM 요소를 초기화할 수 없습니다. 페이지를 새로고침해 주세요.'));
              }
            }
          }, 100);
        };
    ...
    const detectObjects = async () => {
      try {
        ...
        if (!imageRef.current) {
          throw new Error('이미지 요소를 찾을 수 없습니다. 이미지가 올바르게 로드되었는지 확인하세요.');
        }
        
        console.log('이미지 크기:', imageRef.current.naturalWidth, 'x', imageRef.current.naturalHeight);
        ...
        if (imgWidth === 0 || imgHeight === 0) {
          throw new Error('이미지 크기가 0입니다. 이미지가 올바르게 로드되었는지 확인하세요.');
        }
        ...
        // 캔버스 크기 설정
        if (!canvasRef.current || !outputCanvasRef.current || !previewCanvasRef.current) {
          throw new Error('캔버스 요소를 찾을 수 없습니다. DOM이 올바르게 초기화되었는지 확인하세요.');
        }
        ...
        try {
          ...
          if (previewCtx) {
            previewCtx.drawImage(imageRef.current, 0, 0, targetWidth, targetHeight);
          } else {
            console.warn('미리보기 캔버스 컨텍스트를 가져올 수 없습니다.');
          }
        } catch (drawError) {
          console.error('이미지 그리기 오류:', drawError);
          throw new Error(`이미지를 캔버스에 그리는 중 오류가 발생했습니다: ${drawError instanceof Error ? drawError.message : String(drawError)}`);
        }
        ...
        if (predictions.length === 0) {
          throw new Error('이미지에서 객체를 감지할 수 없습니다. 다른 이미지를 시도해보세요.');
        }
    ...
    <div className="w-full">
      {detectionStage === 'detecting' && (
        <div className="mt-4 mb-8 w-full">
          ...
          <p className="text-center mt-2 text-gray-600">객체 감지 중... {progress}%</p>
        </div>
      )}
      ...
      {detectionStage === 'detected' && (
        <div className="mt-4">
          <div className="relative w-full">
            <div id="preview-canvas-container" className="w-full rounded-lg shadow-md">
              <canvas 
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-lg font-medium text-gray-800">감지된 객체</h3>
                <button 
                  onClick={toggleAllObjects}
                  className="px-4 py-1 text-sm bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200"
                >
                  {selectedObjectIds.length === detectedObjects.length ? '모두 해제' : '모두 선택'}
                </button>
              </div>
      ...
      <div id="hidden-elements-container" className="absolute opacity-0 pointer-events-none" style={{ zIndex: -1 }}>
        <img
          ref={imageRef}
          src={imageUrl}
          alt="Original"
