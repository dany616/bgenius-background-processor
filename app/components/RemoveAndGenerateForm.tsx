...
const RemoveAndGenerateForm: React.FC<RemoveAndGenerateFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!apiResponse.ok) {
        ...
        throw new Error(`배경 제거 및 생성에 실패했습니다. 오류 코드: ${apiResponse.status}`);
      }
      
      const data = await apiResponse.json();
      console.log('API 응답 데이터:', data);
      
      if (data && data.bria_results && data.bria_results.result && data.bria_results.result.length > 0) {
        // 모든 이미지 URL을 배열로 추출
        const allImageUrls = data.bria_results.result.map((result: any) => result[0]);
      ...
      const errorMsg = error instanceof Error ? error.message : '이미지 처리 중 오류가 발생했습니다.';
  ...
  return (
    <div className="w-full">
      <h2 className="text-xl font-bold text-gray-800 mb-4">배경 제거 후 배경 생성</h2>
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-white border rounded-lg mb-6">
        <div className="mb-4">
      ...
      {generatedImages.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-800 mb-3">모든 생성된 이미지</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {generatedImages.map((imageUrl, index) => (
              <div 
                key={index}
                className={`border rounded-lg overflow-hidden cursor-pointer transition-all ${
                  selectedImageIndex === index ? 'ring-2 ring-blue-500 scale-105' : 'hover:shadow-md'
                }`}
      ...
      {isLoading && (
        <div className="mt-4 mb-6">
          ...
          <p className="text-center mt-2 text-gray-600">이미지 배경 제거 후 새 배경 생성 중...</p>
        </div>
      )}
    </div>
  );
